...
from enums import GenericQueries, CleanBrowserQueries, OBABrowserQueries, AdvertisementsQueries, ControlVisitsQueries, ControlVisitAdsQueries, AdvertisementsCategoriesQueries, CrawlDataQueries
...
class DataProcesser:
    ...
    def __init__(self, experiment_name: str, webshrinker_credentials):
        ...
        self.experiment_data_path = f'../datadir/{self.experiment_name}/crawl-data.sqlite'
        
        os.makedirs(f'datadir/{self.experiment_name}/results', exist_ok=True)
        self.output_path = f'datadir/{self.experiment_name}/results/chronological_progress.json'
    ...
    def _get_site_ads_from_visit(self, visit_id: AnyStr) -> Tuple[List, List]:
        ...
        pattern = os.path.join(self.source_pages_dir, f'{visit_id}-*.json.gz') # Match all the ads
    ...
    def _set_and_save_static_ads_and_control_sites(self, crawl_cursor, ads_cursor, clear_browser_ids):
        ...
        for index,browser_id in enumerate(clear_browser_ids):
            print(f'Processing static ads of browser {index + 1}/{len(clear_browser_ids)}')
            ...
            static_ads_set = static_ads_set | _get_static_ads_for_visit(clean_run_site_visits)
    ...
    def _get_javascripts_third_party_urls(self, crawl_cursor, browser_id, visit_id, site_url):
        ...
        crawl_cursor.execute(CrawlDataQueries.SelectJavascriptsQuery, (browser_id, visit_id))
        ...
        for javascript in javascripts:
            (js_id, frame_id, script_url, document_url, top_level_url, func_name, call_stack, symbol, operation, value, arguments, time_stamp) = javascript
            urls_in_value = find_urls_in_js(value, top_level_url, script_url, site_url_tld)
        ...
        # Open blocklists
        filter_l_l = [open(path).read().splitlines() for path in self.blocklists_paths_list]
        # Flatten them
    ...
    def _set_and_save_dynamic_ads(self, crawl_cursor, ads_cursor, oba_browser_ids, request_rate=1, taxonomy='iabv1'):
        ...
        async def _async_categorize_ad(landing_ad_url):
            async with semaphore:
                async with aiohttp.ClientSession() as session:
                    response = await self.categorizer.categorize(session, landing_ad_url, taxonomy=taxonomy)
                    ...
                    while True:
                        if response['status_code'] == 200:
                            ...
                            for result in response['categories_response']:
                                ads_cursor.execute(AdvertisementsQueries.SelectAdIdQuery, (landing_ad_url,)) # Single comma for single-element tuple
                                page_id = ads_cursor.fetchone()[0]
                                ads_cursor.execute(
                                    AdvertisementsCategoriesQueries.InsertAdCategoryQuery,
                                    (page_id, landing_ad_url, result['category'], result['taxonomy'], result['taxonomy_tier'], result['taxonomy_id'], result['confident'])
                                )
                            ...
                            print(f"[WARNING] Rate limit exceeded for {landing_ad_url}. Retrying in {delay} seconds...")
        ...
        for index, browser_id in enumerate(oba_browser_ids):
            ...
            print(f'Starting crawled data dynamic ads extraction of browser {index + 1}/{len(oba_browser_ids)}...')
            for control_site_url in self.control_site_urls:
                ...
                for visit_row in control_site_visit_rows:
                    ...
                    # SQLITE
                    ads_cursor.execute(ControlVisitsQueries.InsertControlVisit, (visit_id, browser_id, site_url, site_rank))
                    ads_found_page_source = self._get_site_ads_from_visit(visit_id)
                    possible_ads_js, possible_landing_pages_js = self._get_javascripts_third_party_urls(crawl_cursor, browser_id, visit_id, site_url)
                    ...
                    if ads_found:
                        ...
                        for i in range(len(ads_found[0])):
                            ...
                            # SQLITE
                            try:
                                ads_cursor.execute(AdvertisementsQueries.InsertAdQuery, (landing_ad_url, 0))
                                ads_cursor.execute(AdvertisementsQueries.SelectAdIdQuery, (landing_ad_url,))
                            ...
                            ads_cursor = ads_cursor.execute(AdvertisementsQueries.SelectAdIdQuery,(landing_ad_url,)) # Single comma for single-element tuple
                            ...
                            # SQLITE
                            ads_cursor.execute(ControlVisitAdsQueries.InsertControlVisitAdQuery, (visit_id, control_site_url, site_rank, ad_id, landing_ad_url, href_ad_url))
    ...
    def crawling_data_process(self):
        ...
        self._set_and_save_static_ads_and_control_sites(crawl_cursor, ads_cursor, clear_browser_ids)
...
ads_handler = DataProcesser('airtravel_cookie_banner_yes', webshrinker_credentials=credentials)
