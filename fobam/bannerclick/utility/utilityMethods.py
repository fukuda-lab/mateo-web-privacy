...
def find_optimal(driver: WebDriver, item: tuple, frame=False):
    ...
    while True:
        ...
        if head_el == tail_el:
            optimal_el = head_el
            break
        if head_el.tag_name not in [
            "div",
            "form",
            "section",
        ]:  # another way: check whether there is any sibling for the next one; also can check transparency of head div
            head_el = path.pop(0)
            continue
        if not frame and is_size_equal_to_win(
            driver, head_el
        ):  # one common case is when the fixed element is siezed as the whole body whoever the whole cookie content is usually inside a smaller div
            head_el = path.pop(0)
            continue
        if (opacity != 1 or alpha < 1.0) and is_there_major_child(
            driver, head_el, path
        ):
            head_el = path.pop(0)
            continue
        ...
        if is_one_dimension(
            head_el
        ):  # some divs have whether height or width with zero size
            head_el = path.pop(0)
            continue
...
def find_els_with_cookie(web_comp, lang="en"):
    ...
    els = web_comp.find_elements(
        By.XPATH,
        to_xpath_text(
            concat_with_or([words[lang]["cookies"], words[lang]["cookies1"]])
        ),
    )  # first tries to find the element with "cookies" word which is a great identifier.
    pruning_els(els)
    if (
        not els
    ):  # if no element found, tries with other common words, e.g. {'cookie', 'partner', 'consent', 'accept', ...}
        # els = web_comp.find_elements_by_xpath("//*[text()[contains(., 'cookie') and (contains(., 'partners') or contains(., 'consent') or contains(., 'accept'))]]")
        temp_str = concat_with_and(
            [
                words[lang]["cookie"],
                concat_with_or(
                    [
            ...
            els = web_comp.find_elements(
                ...
                to_xpath_text(
                    concat_with_or(
                        [
                            words[lang]["cookie"],
                            words[lang]["privacy policy"],
                            words[lang]["legitimate interest"],
                        ]
                    )
                ),
            )
...
def clear_fill_input(driver, input, value):
    ...
    action.click(input).key_down(Keys.CONTROL).send_keys("a").key_up(
        Keys.CONTROL
    ...
    try:
        ...
        for p in all_p:
            ...
            if res:
                ...
                res = any(
                    word in t_text for word in find_variations_of_word("privacy policy")
                )
                if res:
                    els_with_cookies.append(p)
                else:
                    res = any(
                        word in t_text for word in find_variations_of_word("accept")
                    ) or any(
                        word in t_text for word in find_variations_of_word("cookie")
                    )
                    if res:
                        if (
                            any(
                                word in t_text
                                for word in find_variations_of_word("partner")
                            )
                            or any(
                                word in t_text
                                for word in find_variations_of_word("legitimate")
                            )
                            or any(
                                word in t_text
                                for word in find_variations_of_word("personal data")
                            )
                        ):
                            els_with_cookies.append(p)
            ...
            if els_with_cookie:
                ...
                banner = find_optimal(
                    ...
                    (
                        ...
                        find_deepest_el(els_with_cookie),
                    ),
                    frame=True,
                )
                ...
                child_iframes = driver.find_elements(
                    By.TAG_NAME, "iframe"
                )  # just one level checking {iframe in iframe}
...
def page_lang(
    driver: WebDriver, try_flag=True
):  # first use cld3 lib to detect the language if failed uses lang attr of HTML tag.
    try:
        lang = None
...
def translate_page(driver: WebDriver):
    driver.get(
        "https://translate.google.com/translate?sl=auto&tl=en&u=" + driver.current_url
    )
    # iframes = driver.find_elements_by_tag_name("iframe")
    iframes = driver.find_elements(By.TAG_NAME, "iframe")
    if iframes:
...
def click_on_btns(btns: list[WebElement], file_name, sc):
    ...
    for j, btn in enumerate(btns):
        ...
        if flag:
            ...
            flag = click_and_check(
                find_parent(btn), file_name
...
def if_btn_clicked(btn):
    return (
        ...
        or not is_inside_viewport(btn)
        or not clickable(btn)
    )
