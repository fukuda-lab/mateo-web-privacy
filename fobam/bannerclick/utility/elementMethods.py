...
def is_there_major_child(
    driver: WebDriver, el: WebElement, path: list[WebElement]
):  # return true if there is a child in path whose size is larger than el
    for child in path:
        if (
            child.text == el.text
            and (
                calc_area(list(el.size.values())) - calc_area(list(child.size.values()))
            )
            > calc_area(list(get_win_inner_size(driver))) / 10.0
            and not is_one_dimension(child)
        ):
            # if has_img(el):
            #     if has_img(child):
            #         return True
...
def find_common_ancestor_for_list(els: list[WebElement]):
    if els is None:
        warnings.warn(
            'warning in "find_common_ancestor_for_list" method', DeprecationWarning
        )
    elif len(els) == 1:
        return els[0]
    elif len(els) == 2:
...
def is_inside_ellipse(center: tuple, edges: tuple, point: tuple, tolerance: float):
    return (math.sqrt(point[0] - center[0]) / math.sqrt(edges[0] * tolerance)) + (
        math.sqrt(point[1] - center[1]) / math.sqrt(edges[1] * tolerance)
    ) < 1
...
def find_major_ancestor(
    el: WebElement,
):  # major ancestor is the common one with the most Inclusivity
    temp_el = el
    if is_fixed_element(temp_el):
        ...
        while True:
            ...
            if is_inside_ellipse(par_loc, par_size, el_loc, 0.2) and is_inside_ellipse(
                [sum(x) for x in zip(par_loc, par_size)],
                par_size,
                [sum(x) for x in zip(el_loc, el_size)],
                0.2,
            ):
                temp_el = parent
            else:  # todo
...
def pruning_btns(
    els: list[WebElement],
):  # delete some unrelated els, like those that are options which usually are found in footers and headers of a webpage, and also rules out script elements.
    unrelated_btns = []
    for el in els:
        try:
            if is_wordy(el):
...
def del_invisible_els(els: list[WebElement]):
    ...
    for el in els:
        ...
        try:
            while True:
                if temp_el.value_of_css_property("display") == "none" or is_neg_zindex(
                    el
                ):
...
def if_contains_words(el: WebElement, words, check_attr=True):
    for word in words:
        if word in el.text.lower() or (
            check_attr and html_attr_contains_words(el, words)
        ):
            return True
    return False
...
def del_unrelated_els(
    els: list[WebElement], strict
):  # delete some unrelated els, like those that are options which usually are found in footers and headers of a webpage, and also rules out script elements.
    unrelated_elements = []
    for el in els:
        try:
...
def find_path(
    el1: WebElement, el2: WebElement
):  # find the sequential elements between to element in the DOM.
    path = []
    temp_el = el2
    while True:
