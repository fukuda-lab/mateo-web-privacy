...
def get___tcfapiLocator():
    ...
    return driver.find_elements(
        By.XPATH, "//iframe[contains(@name, '__tcfapiLocator')]"
    )
...
def extract_CMP_list_from_server():
    ...
    driver.find_element(
        By.XPATH, "//*[contains(text(), 'TCF v2.0 CMP Service')]"
    ).click()

    # select_element = driver.find_element_by_name('tablepress-26_length')
    select_element = driver.find_element(By.NAME, "tablepress-26_length")
    ...
    driver.find_element(
        By.XPATH, "//*[contains(text(), 'TCF v2.0 CMP Private (own use)')]"
    ).click()

    # select_element = driver.find_element_by_name('tablepress-78_length')
    select_element = driver.find_element(By.NAME, "tablepress-78_length")
...
def run_cmp_detection():
    try:
        ...
        with open(log_file, "a+") as f:
            print(
                "failed to continue detecting CMP for domain: "
                + this_domain
                + " "
                + ex.__str__(),
                file=f,
...
def init(
    web_driver=None, db=None
):  # initialize cmpdetection by setting webdriver instance and database
    global driver
    if web_driver is None:
