...
def init(
    ...
    b_db=None,
    h_db=None,
):  # initialize bannerdetection by setting url file and webdriver instance
    ...
    if input_file:
        file = input_file
    init_str = (
        f"""Crawl initialized for: {file} in {datetime.now().strftime("%H-%M-%S").__str__()}
...
def open_domain_page(domain, sleep=TEST_MODE_SLEEP):
    ...
    while True:
        ...
        try:
            ...
            with open(log_file, "a+") as f:
                print(
                    "failed to get (unreachable): " + url + " " + ex.__str__(), file=f
                )
            this_status = 2
        finally:
            mode += 1
...
def find_cookie_banners(translate=False, stale_flag=False):
    ...
    try:
        ...
        if translate:
            ...
            detected_lang = "en"
            els_with_cookie = find_els_with_cookie(
                body_el
        ...
        if els_with_cookie:
            ...
            if not banners_map:
                ...
                banners_map[driver.find_element(By.TAG_NAME, "body")] = find_deepest_el(
                    els_with_cookie
                )
        ...
        frame_pairs = find_CMP_cookies_iframes(
            driver, detected_lang
...
def detect_banners(data):  # return banners of the current running url
    ...
    try:
        ...
        if not banners and TRANSLATION:
            if "en" not in this_lang and is_in_langlist(
                this_lang
        ...
        with open(log_file, "a+") as f:
            print(
                "failed to continue detecting banner for domain: "
                + this_domain
                + " "
                + ex.__str__(),
                file=f,
...
def open_domain_plus_detect_banner(
    domain,
...
def interact_with_banner(banner_item, choice, status, i, total_search=False):
    ...
    try:
        ...
        with open(log_file, "a+") as f:
            print(
                "failed in switching frame for : "
                + this_url
                + " in interact with banner. "
                + ex.__str__(),
                file=f,
            )
    ...
    try:
        if (
            total_search
        ):  # search the whole body DOM for the words, this is because sometimes for example after clicking on setting the banner DOM disappears or the page redirect to another page.
            el = body_el
        else:
            el = banner
    except:
    ...
    try:
        ...
        with open(log_file, "a+") as f:
            print(
                "failed in interact with banner for : "
                + this_url
                + "  "
                + ex.__str__(),
                file=f,
...
def take_current_page_sc(data=None, directory=None, suffix=""):
    ...
    if SCREENSHOT:
        ...
        try:
            ...
            with open(log_file, "a+") as f:
                print(
                    "failed to take screenshot for domain: "
                    + data.domain
                    + " "
                    + ex.__str__(),
                    file=f,
...
def take_banner_sc(banner_item, data, j=None):
    if banner_item:
        try:
            ...
            with open(log_file, "a+") as f:
                print(
                    "failed in switching in banner_sc section for : "
                    + this_url
                    + " "
                    + ex.__str__(),
                    file=f,
...
def chrome_element_sc(banner, index, j):
    ...
    crop_image.save(
        sc_dir + get_sc_file_name(index) + "_banner" + str(j + 1) + "_ch.png"
    )
...
def extract_banner_data(banner_item):
    ...
    try:
        banner_data["captured_area"] = calc_area(
            list(banner.size.values())
        ) / calc_area(list(get_win_inner_size(driver)))
        ...
        with open(log_file, "a+") as f:
            print(
                "failed in extracting banner for : " + this_url + " " + ex.__str__(),
                file=f,
            )
        return
...
def get_data_dicts(banner_data):
    ...
    try:
        ...
        with open(log_file, "a+") as f:
            print(
                "failed to continue extracting banner data for domain: "
                + this_url
                + " "
                + ex.__str__(),
                file=f,
    ...
    if banners:
        for j, banner_item in enumerate(banners):
            try:
                ...
                with open(log_file, "a+") as f:
                    print(
                        "failed to continue in taking banner sc for domain: "
                        + this_url
                        + " "
                        + ex.__str__(),
                        file=f,
...
def set_data_in_db_error(data):
    ...
    try:
        ...
        with open(log_file, "a+") as f:
            print(
                "failed to continue setting data in DB for domain: "
                + this_domain
                + " "
                + ex.__str__(),
                file=f,
...
def set_data_in_db(data):
    ...
    v_dict = {
        ...
        "lang": this_lang,
        "banners": num_banners,
        "btn_status": data.btn_status["btn_status"],
        "btn_set_status": data.btn_status["btn_set_status"],
...
def run_all_for_domain(DMN, URL):
    ...
    try:
        ...
        with open(log_file, "a+") as f:
            print("Memory Error happened for: " + DMN + "  " + ex.__str__(), file=f)
    except InvalidSessionIdException as ex:
        visit_db.loc[visit_db.index[-1], "status"] = -1
        with open(log_file, "a+") as f:
            print(
                "InvalidSessionIdException happened for: " + DMN + "  " + ex.__str__(),
                file=f,
            )
        raise
    except Exception as ex:
        visit_db.loc[visit_db.index[-1], "status"] = -1
        with open(log_file, "a+") as f:
            print("Exception happened for: " + DMN + "  " + ex.__str__(), file=f)
    # finally:
    #     if not (counter % 100):
