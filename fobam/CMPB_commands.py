...
from bannerclick.config import MOBILE_AGENT, log_file
...
class CustomBannerInteraction(BaseCommand):
    """
    run all the Get and Bannerdetection with Interaction in one single command without saving all of the cookie banner data in SQLite DB
    """

    def __init__(self, url, sleep, index, timeout, choice, result_csv_file_name):
        ...
        self.choice = choice
        self.result_csv_file_name = result_csv_file_name

    def __repr__(self):
        return "CustomBannerInteraction({},{},{},{})".format(
    ...
    def execute(
        ...
        try:
            ...
            # # Run banner detection and interaction
                ...
                try:
                    ...
                    with open(self.result_csv_file_name, "a") as f:
                        f.write(f"{self.url},{1 if len(banners) else 0}\n")
                except Exception as e:
                    print(f"Error writing to csv file: {e}")
                # Add sleep after interaction to give time to ads aggregators
                time.sleep(self.sleep)
                # Mateo: commented because we don't care about Consent Management Platform or saving the banner info in the db
        ...
        if self.choice == 0:
            ...
            self.logger.info(
                "CMPB command is successfully executed and result for {} is: number of banners {}.".format(
                    current_url, len(banners)
                )
            )
...
class CMPBCommand(BaseCommand):
    ...
    def execute(
        ...
        if self.choice == 0:
            ...
            self.logger.info(
                "CMPB command is successfully executed and result for {} is: number of banners {} and CMP existance {}.".format(
...
class BannerDetectionCommand(BaseCommand):
    ...
    def execute(
        ...
        Data.banners = banners
        self.logger.info(
            "BannerDetection command is successfully executed and there is %d banners for: %s",
            len(banners),
            current_url,
        )
